...
impl ProtocolPacket {
    ...
    pub fn decode_packet(packet: &[u8]) -> Result<ProtocolPacket> {
        if packet.len() < 4 {
            anyhow::bail!("Packet is too short (decode_packet)");
        }
        ...
        Ok(ProtocolPacket {
            ...
            decoded: match packet[CMD_OFFSET] {
                0x02 if packet[SRC_OFFSET] == 0x10 && packet[DEST_OFFSET] == 0x0f => {
                    PacketType::Status(system_state::SystemState::from_packet(packet)?)
                }
